...
for m in np.arange(cross_index.shape[0]):
    ...
    for k in np.arange(1,traj_lon.shape[0]-1):
        ...
        if (mpl.path.Path(vertices=vert).intersects_path(PATH, filled=True)) == True:
            ...
            y_200_loc = np.append(arr=y_200_loc, values = y)
            x_200_loc = np.append(arr=x_200_loc, values = x)
...
np.save(file='/data/ssahu/NEP36_2013_summer_hindcast/Ariane_files/x_200_total_all_down_multiple.npy', arr=x_200_loc)
np.save(file='/data/ssahu/NEP36_2013_summer_hindcast/Ariane_files/y_200_total_all_down_multiple.npy', arr=y_200_loc)
